...
CAMSV_CUSTOM_BUFFER_INFO getAeBufSize_ov64b40mipiraw();
CAMSV_CUSTOM_BUFFER_INFO getFlkBufsize_ov64b40mipiraw();
UINT32 DecodeAe_ov64b40mipiraw(VOID* const pData);
UINT32 DecodeFlk_ov64b40mipiraw(VOID* const pData);
...
impCamsvBufSize(CAMSV_CUSTOM_DECODE_ENUM const CamsvCusDecodeFeature) const
{
    ...
    switch(CamsvCusDecodeFeature)
    {
        case CAMSV_CUSTOM_DECODE_AE:
            buf_size = getAeBufSize_ov64b40mipiraw();
            break;
        case CAMSV_CUSTOM_DECODE_FLK:
            buf_size = getFlkBufsize_ov64b40mipiraw();
            break;
        default:
...
impCamsvCustomDecode(CAMSV_CUSTOM_DECODE_ENUM const CamsvCusDecodeFeature, VOID* const pData) const
{
    ...
    {
        case CAMSV_CUSTOM_DECODE_AE:
            DecodeAe_ov64b40mipiraw(pData);
            break;
        case CAMSV_CUSTOM_DECODE_FLK:
            DecodeFlk_ov64b40mipiraw(pData);
            break;
        default:
...
UINT32 DecodeAe_ov64b40mipiraw(VOID* const pData)
{
    ...
    CAMSV_CUSTOM_BUFFER_INFO buf = getAeBufSize_ov64b40mipiraw();
    memset(pOutput, 0, buf.size * sizeof(MUINT32));
    MUINT32 u4AEHistTotalSum = 6209280; // Default for OV64C40
...
UINT32 DecodeFlk_ov64b40mipiraw(VOID* const pData)
{
    ...
    CAMSV_CUSTOM_BUFFER_INFO buf = getFlkBufsize_ov64b40mipiraw();
    memset(pOutput, 0, buf.size * sizeof(MUINT32));
